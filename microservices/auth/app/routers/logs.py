...
from models import LogCreate, LogOut, LogListOut
...
@router.post("/", response_model=LogOut, dependencies=[Depends(require_admin)])
async def create_log(payload: LogCreate, request: Request):
    async with request.app.state.pool.acquire() as conn:
        row = await conn.fetchrow(
            """
            INSERT INTO system_log (user_id, level, action, path, ip, user_agent, meta)
            VALUES ($1, $2, $3, $4, $5, $6, $7)
            RETURNING id, user_id, level, action, path, ip::text AS ip, user_agent, meta, created_at
            """,
